...
| **Is that block actually a spread leg?** | `/api/option-trades/multi-leg`, `/api/option-trades/multi-leg/{id}/legs` | [`pitfalls/32-multi-leg-share-before-block-direction.md`](pitfalls/32-multi-leg-share-before-block-direction.md), applied before reading direction — these two endpoints make the filter *exact* instead of a share estimate |

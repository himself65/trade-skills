...
> **Scope limit — IVR selects the vega *side*, it does not certify *edge*.** Everything in this rule is a statement about which sign of vega to prefer, derived from mean reversion inside the name's own distribution. It is **not** a claim that the credit is worth collecting: a name can sit at high IVR and still be a losing sale because implied keeps landing below subsequent realized, and can sit at low IVR and be a fine sale because forward realized collapses. Before acting on the short-vega half of this rule, run [pitfall 36](36-ivr-ranks-vol-vrp-prices-it.md) — the variance risk premium is the instrument for that question.
...
3. **Sanity-check the net vega sign before submitting**: long-vega structure at IVR <30 ✓, short-vega at IVR >70 ✓. Anything else needs an explicit written reason (e.g., "I expect IV to compress further because X").
   - **If the structure is short vega, the IVR check is necessary but not sufficient** — add the forward-aligned IV-vs-subsequent-realized check from [pitfall 36](36-ivr-ranks-vol-vrp-prices-it.md) before the credit is treated as edge.
...
**Cross-references**:
- [Pitfall 36](36-ivr-ranks-vol-vrp-prices-it.md) — **completes this rule**: IVR picks the vega side, the variance risk premium decides whether selling that side has edge, and the IV *level* sets what you collect

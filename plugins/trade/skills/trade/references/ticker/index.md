...
description: Lookup index of closed/in-progress trade post-mortems (INTC, Mag-7, APP, NOK, TSEM, CBRS, SNOW, MDB, VIX, SATS, 6981, MU, NQ, NBIS, BE); load full files by pattern.
...
| BE | Pre-earnings vol-side study at a 1-year IV floor (T−55 → T−38) | 2026-09-02 (open) | no trade yet (structure study); analyst-error corrected mid-thread | **At an IV floor with a print 55 days out the vega side is LONG, and the analysis defaulted to vega-neutral on a post-crush trailing VRP.** The name's own last four prints ramped IV30 +14/+41/+42/+64 into T−1 with the pre-event forward VRP negative in 3/4; the reflex fix (buy the event month) needed ~168% IV at flat spot to break even, so the expression became carrier + clock + conversion — vega 2–3 expiries out or a no-event/event calendar with positive carry, converted to a spread at T−1. Long delta + long vega = the bull call spread split in time. | `be-2026-09.md` |

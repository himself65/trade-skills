...
This is the skill payload (`skills/trade/`) inside the parent repo. For repo-level install instructions and catalog overview, see `../../README.md`.

The skill auto-triggers on trading-related prompts (per the `description` in `SKILL.md` frontmatter), or can be invoked explicitly via `/trade`.

`CLAUDE.md` is kept as a thin `@SKILL.md` import so that working *inside* this skill directory automatically loads the same content as project memory.
